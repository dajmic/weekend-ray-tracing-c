	...
	for (int j = 0; j < cam.image_height; j++) {
		...
		for (int i = 0; i < cam.image_width; i++) {
			...
			for (int sample = 0; sample < cam.samples_per_pixel; sample++) {
				...
				pixel_colour =
					v3_add(pixel_colour, ray_colour(r, cam.max_depth, world));
			}
...
Camera camera()
{
	...
	double viewport_width =
		viewport_height * ((double)cam.image_width / image_height);
	...
	Point3 horizontal_adjust =
		v3_subtract(focal_length_adjust, v3_scale(viewport_u, 0.5));
	Point3 viewport_upper_left =
		v3_subtract(horizontal_adjust, v3_scale(viewport_v, 0.5));
	cam.pixel_upper_left =
		v3_add(viewport_upper_left,
			   v3_scale(v3_add(cam.pixel_delta_u, cam.pixel_delta_v), 0.5));
...
static Colour ray_colour(Ray r, int max_depth, SphereList *world)
{
	...
	if (sphere_list_hit(world, r, interval(0.0, INFINITY), &hit_rec)) {
		...
		return v3_scale(
			ray_colour(ray(hit_rec.point, direction), max_depth, world), 0.5);
	}

	// Background gradient.
	Vec3 unit_direction = v3_unit(r.direction);
	double a = 0.5 * (unit_direction.y + 1.0);
	return v3_add(v3_scale(colour(1.0, 1.0, 1.0), (1.0 - a)),
				  v3_scale(colour(0.5, 0.7, 1.0), a));
}
...
static Vec3 sample_square()
{
	...
	// square.
	return (
		Vec3){ .x = random_double() - 0.5, .y = random_double() - 0.5, .z = 0 };
}
...
static Ray get_ray(Camera *cam, int i, int j)
{
	...
	Point3 pixel_sample =
		v3_add(v3_add(cam->pixel_upper_left, offset_x), offset_y);

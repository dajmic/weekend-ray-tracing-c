...
vec3 init_v3_random_in_range(double min, double max) {
  return init_v3(random_double_in_range(min, max),
...
vec3 random_on_hemisphere(vec3 normal) {
  ...
  if (v3_dot(on_unit_sphere, normal) >
      0.0) // In the same hemisphere as the normal vector.
  {
...
vec3 v3_neg(vec3 v) { return v3_scale(v, -1); }
